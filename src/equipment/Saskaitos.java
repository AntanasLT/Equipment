...
//	    filter();
	...
	select = SELECT + "WHERE SUBSTR(s.Data, 1, 4) = b.Metai AND DATEDIFF(CURDATE(), s.Data) < 365 ORDER BY s.Data DESC";
...
//	    sb.append(" WHERE SUBSTR(s.UzsData, 1, 4) = b.Metai AND ");
	...
	sb.append(" GROUP BY NR ORDER BY s.Data");
...
//            preparedFilter.setInt(n, Integer.valueOf(kontrahentai[0][cbKontrah.getSelectedIndex()]));
	...
	ResultSet resultset;
	saskSuma = 0;
//	System.out.print("filter(String query)"); System.out.println(query);
	try {
...
//                cbBiuPavad.setSelectedIndex(cbBiuKodai.getSelectedIndex());
	...
	if (me.getButton() == 3) {
	    if (me.getComponent().equals(tfIvesta)) {
		tfIvesta.setText(date.getToday());
	    }
	    if (me.getComponent().equals(tfDate)) {
		tfDate.setText(date.getToday());
	    }
	    if (me.getComponent().equals(tfUzsData)) {
		tfUzsData.setText(date.getToday());
	    }
	}
    }
