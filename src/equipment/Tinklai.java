...
// UPDATE Tinklai SET Free = ?, MAC = ?, Device = ?, Location = ?, User = ?, Password = ?, Room = ?, Plint = ?, Switch = ?, Port = ?, Pastaba = ? WHERE IP = ?
	...
	row = table.getSelectedRow();
        pastaba = taMessage.hasFocus() ? taMessage.getText() : (String) table.getValueAt(row, 11);
	if (row >= 0) {
	    try {
		if (preparedUpdate == null) {
		    preparedUpdate = connection.prepareStatement(PREPARE_UPDATE);
                ...
                for (i = 1; i < FIELDS_COUNT-1; i++) {
                    preparedUpdate.setString(i, (String) table.getValueAt(row, i));
                }
                preparedUpdate.setString(11, pastaba);
                preparedUpdate.setString(12, (String) table.getValueAt(row, 0));
		...
		if (preparedInsert == null) {
		    preparedInsert = connection.prepareStatement(PREPARE_INSERT);
                ...
                for (i = 1; i < FIELDS_COUNT; i++) {
                    preparedInsert.setString(i, (String) table.getValueAt(row, i-1));
                }
                pastaba = taMessage.requestFocusInWindow() ? taMessage.getText() : (String) table.getValueAt(row, 11);
